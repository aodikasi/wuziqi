$(document).ready(function(){
    ...
    $("#start-game").on('click',function(){
        try{
            if(game.white && game.white.worker) game.white.worker.terminate();
            if(game.black && game.black.worker) game.black.worker.terminate();
        }catch(e){}
        if(gameData.mode==='vshuman'){
            game.mode='hvh';
            game.init(new HumanPlayer("black"), new HumanPlayer("white"));
        }else if(gameData.mode==='vsai'){
            game.mode=gameData.level;
            game.init(new AIPlayer(game.mode, "black"), new AIPlayer(game.mode, "white"));
        }else{
            var color, other;
